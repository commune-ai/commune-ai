...
def get_trainer(trainer_path):
    ...
    submodule = import_module(f"trainer.{trainer_module_path}")
    return getattr(submodule, trainer_class)


def get_env(trainer_path):
    trainer_class = trainer_path.split(".")[-1]
    trainer_module_path = ".".join(trainer_path.split(".")[:-1])
    submodule = import_module(f"data.envs.{trainer_module_path}")
    return getattr(submodule, trainer_class)
...
ModelPortfolioinput_dict,keys, value ):
    """
...
def dict_has(input_dict,keys):
    ...
    tmp_input_dict = deepcopy(input_dict)

    has_key = True
    for key in keys:
        if key in tmp_input_dict:
            tmp_input_dict = tmp_input_dict[key]
        else:
            has_key = False
...
def dict_flat(input_dict, root_key = ''):
    if isinstance(input_dict, dict):
        output_dict = {}

        for k,v in input_dict.items():
            new_root_key = f'{root_key}.{k}'
            output_dict_flat = dict_flat(input_dict=v, root_key=new_root_key)
            if isinstance(output_dict_flat,dict):
                output_dict.update(output_dict_flat)
            else:
                dict_put(output_dict, keys=new_root_key, value=output_dict_flat)
