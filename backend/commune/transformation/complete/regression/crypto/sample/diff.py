...
class SampleTransformManager(object):
    def __init__(self,
        ...
        self.index_bounds = [
            0,
            self.periods["input"],
            self.periods["input"] + self.periods['output']
        ]
        self.build()
    def build(self):
        """
        ...
        """
        if self.process_pipeline_map is None:
            self.process_pipeline_map = {}
        # price sample wise preprocessing for prices
        for key in self.feature_group['con']:
            if key not in self.process_pipeline_map:
                self.process_pipeline_map[key] =  \
                    {'pipeline':  SequentialTransformPipeline(pipeline=[

                        savgol_filter_transform(window_length=5,
                                                polyorder=3),
