...
class NBEATSSeasonalBlock(NBEATSBlock):
    def __init__(
        ...
        self.S_backcast = torch.cat([s1_b, s2_b])
        ...
        self.S_forecast = torch.cat([s1_f, s2_f])
    ...
    def forward(self, x) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        backcast = torch.einsum('...ij, jk-> ...ik', amplitudes_backward, self.S_backcast.to(local_device))
        amplitudes_forward = self.theta_f_fc(x)
        forecast = torch.einsum('...ij, jk-> ...ik', amplitudes_forward, self.S_forecast.to(local_device))
...
class NBEATSTrendBlock(NBEATSBlock):
    ...
    def forward(self, x) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        backcast = torch.einsum('...ij, jk-> ...ik', theta_b, self.T_backcast.to(local_device))
        forecast = torch.einsum('...ij, jk-> ...ik', theta_f, self.T_forecast.to(local_device))
